...
function createCost(req, res, next) {
    if (!req.body.name) return next({status: 400, message: `Costume must have a name`})
...
function deleteTag(req, res, next) {
  ...
  if (deleted.error) return next({status: 404, message: deleted.error})
  res.status(200).send(deleted)
}
function updateTag(req, res, next) {
    const updated = model.updateTag(req.params.id, req.params.tagId, req.body.name)
    if (updated.error) return next({status: 404, message: updated.error})
    res.status(201).send(updated)
}

module.exports =  {getAllCost, getOneCost, createCost, updateCost, deleteCost, getAllTags, getOneTag, createTag, deleteTag, updateTag } 